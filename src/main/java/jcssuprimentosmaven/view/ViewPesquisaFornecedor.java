...
import jcssuprimentosmaven.ouvinte.OuvinteViewDadosFornecedor;
 ...
 * @author Gustavo
 */
public class ViewPesquisaFornecedor extends javax.swing.JInternalFrame {
    ...
    private void abrirViewDadosFornecedor(Fornecedor fornecedor, String acao) {
        //if (viewDadosFornecedor == null) {
            ...
            this.getParent().add(viewDadosFornecedor);
            viewDadosFornecedor.setPosicao();
        //}
        
         if (acao.equals("novo")) {
            viewDadosFornecedor.bEditar.setEnabled(false);
            viewDadosFornecedor.bExcluir.setEnabled(false);
            viewDadosFornecedor.bGravar.setEnabled(true);
        }
        if (acao.equals("editar")) {
            viewDadosFornecedor.bGravar.setEnabled(false);
            viewDadosFornecedor.bExcluir.setEnabled(false);
            viewDadosFornecedor.bEditar.setEnabled(true);
        }
        if (acao.equals("excluir")) {
            viewDadosFornecedor.bExcluir.setEnabled(true);
            viewDadosFornecedor.bEditar.setEnabled(false);
            viewDadosFornecedor.bGravar.setEnabled(false);
        }

        viewDadosFornecedor.setFornecedor(fornecedor);
        viewDadosFornecedor.exibirFornecedor(viewDadosFornecedor.getFornecedor());
        //viewDadosJuiz.preencherJuiz();
        viewDadosFornecedor.setVisible(true);
        try {
            viewDadosFornecedor.setSelected(true);
        } catch (PropertyVetoException ex) {
            ViewUtil.addMsgErro("Não foi possível abrir a ViewDadosJuiz");
        }
    }

    public void listar(List fornecedores) {
        try {
            this.fornecedores = fornecedores;
            DefaultTableModel model = (DefaultTableModel) tJuizes.getModel();
            this.removerLinhasDaTabela(model);

            Iterator resultado = fornecedores.iterator();

            while (resultado.hasNext()) {
                Fornecedor fornecedorTemp = (Fornecedor) resultado.next();
                Long id = fornecedorTemp.getId();
                String nome = fornecedorTemp.getFantasia();
                String empresa = fornecedorTemp.getEmpresa().getNomeFantasia();
            ...
            ViewUtil.addMsgErro("Ocorreu um erro ao carregar os fornecedores " + ex.getMessage());
        }
    }
    ...
    public Fornecedor getFornecedor() {
        Fornecedor fornecedorTemp = null;
        try {
            ...
            fornecedorTemp = (Fornecedor) this.fornecedores.get(linhaSelecionada);

        } catch (RuntimeException ex) {
            ViewUtil.addMsgErro("Erro ao carregar o Juiz " + ex.getMessage());
        }
    ...
    private void bNovoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bNovoActionPerformed
        ...
        this.abrirViewDadosFornecedor(fornecedor, "novo");
    ...
    private void bAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bAlterarActionPerformed
        ...
        try {
            ...
            this.abrirViewDadosFornecedor(fornecedor, "editar");
