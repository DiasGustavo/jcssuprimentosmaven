...
import jcssuprimentosmaven.ouvinte.OuvinteViewDadosFabrica;
 ...
 * @author Gustavo
 */
public class ViewPesquisaFabrica extends javax.swing.JInternalFrame {

    private ViewDadosFabrica viewDadosFabrica;
     ...
     */
    public ViewPesquisaFabrica(Jogador jogador) {
        initComponents();
    ...
    private void abrirViewDadosFabrica(Fabrica fabrica, String acao) {
       
            viewDadosFabrica = new ViewDadosFabrica();
            OuvinteViewDadosFabrica ouvinte = new OuvinteViewDadosFabrica(viewDadosFabrica);
            viewDadosFabrica.setAcao(acao);            
            this.getParent().add(viewDadosFabrica);
            viewDadosFabrica.setPosicao();        
            
        if (acao.equals("novo")) {
            viewDadosFabrica.bEditar.setEnabled(false);
            viewDadosFabrica.bExcluir.setEnabled(false);
            viewDadosFabrica.bGravar.setEnabled(true);
            viewDadosFabrica.bNovo.setEnabled(true);
        }
        if(this.jogador.getFuncao()==1){
            viewDadosFabrica.bEditar.setEnabled(false);
            viewDadosFabrica.bGravar.setEnabled(false);
            viewDadosFabrica.bExcluir.setEnabled(false);
            viewDadosFabrica.bNovo.setEnabled(false);            
        }else{
            if (acao.equals("editar")) {
                viewDadosFabrica.bGravar.setEnabled(false);
                viewDadosFabrica.bExcluir.setEnabled(false);
                viewDadosFabrica.bEditar.setEnabled(true);
                viewDadosFabrica.bNovo.setEnabled(false);
            }
        }
        
        if (acao.equals("excluir")) {
            viewDadosFabrica.bExcluir.setEnabled(true);
            viewDadosFabrica.bEditar.setEnabled(false);
            viewDadosFabrica.bGravar.setEnabled(false);
            viewDadosFabrica.bNovo.setEnabled(false);
        }

        viewDadosFabrica.setFabrica(fabrica);
        viewDadosFabrica.exibirFabrica(viewDadosFabrica.getFabrica());
        //viewDadosJuiz.preencherJuiz();
        viewDadosFabrica.setVisible(true);
        try {
            viewDadosFabrica.setSelected(true);
        } catch (PropertyVetoException ex) {
            ViewUtil.addMsgErro("Não foi possível abrir a viewDadosFabrica");
        }
    }

    public void listar(List fabricas) {
        try {
            this.fabricas = fabricas;
            DefaultTableModel model = (DefaultTableModel) tFabricas.getModel();
            this.removerLinhasDaTabela(model);

            Iterator resultado = fabricas.iterator();

            while (resultado.hasNext()) {
                Fabrica fabricaTemp = (Fabrica) resultado.next();
                Long id = fabricaTemp.getId();
                String nome = fabricaTemp.getNomeFantasia();                
                String empresa = fabricaTemp.getEmpresa().getNomeFantasia();
            ...
            ViewUtil.addMsgErro("Ocorreu um erro ao carregar as Fabricas " + ex.getMessage());
        }
    }
    ...
    public Fabrica getFabrica() {
        Fabrica fabricaTemp = null;
        try {

            int linhaSelecionada = tFabricas.getSelectedRow();
            if (linhaSelecionada < 0) {
                ViewUtil.addMsgErro("Nenhuma linha foi Selecionada");
            }
            fabricaTemp = (Fabrica) this.fabricas.get(linhaSelecionada);

        } catch (RuntimeException ex) {
            ViewUtil.addMsgErro("Erro ao carregar o Fabrica " + ex.getMessage());
        }
        return fabricaTemp;
    }
    ...
    private void initComponents() {
        ...
        tFabricas = new javax.swing.JTable();
        ...
        tFabricas.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        tFabricas.getTableHeader().setReorderingAllowed(false);
        spListaJuizes.setViewportView(tFabricas);
    ...
    private void bNovoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bNovoActionPerformed
        ...
        fabrica.setCustoTransformacao(BigDecimal.ZERO);
        fabrica.setFatorTransformacao(BigDecimal.ZERO);
        fabrica.setPesquisa(BigDecimal.ZERO);
        fabrica.setTaxaServicoCliente(BigDecimal.ZERO);
        
        this.abrirViewDadosFabrica(fabrica, "novo");
    ...
    private void bAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bAlterarActionPerformed
        ...
        try {
            ...
            this.abrirViewDadosFabrica(fabrica, "editar");

        } catch (RuntimeException ex) {
            ViewUtil.addMsgErro("Não foi possível carregar os dados da Fabrica " + ex.getMessage());
        }
    ...
    private javax.swing.JTable tFabricas;
