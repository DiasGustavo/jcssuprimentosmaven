...
import jcssuprimentosmaven.ouvinte.OuvinteViewDadosJuiz;
 ...
 * @author Gustavo
 */
public class ViewPesquisaJuiz extends javax.swing.JInternalFrame {

    private ViewDadosJuiz viewDadosJuiz;
    private List juizes;

    /**
     * Creates new form ViewPesquisaJuiz
     */
    public ViewPesquisaJuiz() {
        initComponents();
    }
    ...
    private void abrirViewDadosJuiz(Juiz juiz, String acao) {
        if (viewDadosJuiz == null) {
            viewDadosJuiz = new ViewDadosJuiz();
            OuvinteViewDadosJuiz ouvinte = new OuvinteViewDadosJuiz(viewDadosJuiz);
            this.getParent().add(viewDadosJuiz);
            viewDadosJuiz.setPosicao();
        }
        
         if (acao.equals("novo")) {
            viewDadosJuiz.bEditar.setEnabled(false);
            viewDadosJuiz.bExcluir.setEnabled(false);
            viewDadosJuiz.bGravar.setEnabled(true);
        }
        if (acao.equals("editar")) {
            viewDadosJuiz.bGravar.setEnabled(false);
            viewDadosJuiz.bExcluir.setEnabled(false);
            viewDadosJuiz.bEditar.setEnabled(true);
        }
        if (acao.equals("excluir")) {
            viewDadosJuiz.bExcluir.setEnabled(true);
            viewDadosJuiz.bEditar.setEnabled(false);
            viewDadosJuiz.bGravar.setEnabled(false);
        }

        viewDadosJuiz.setJuiz(juiz);
        viewDadosJuiz.exibirJuiz(viewDadosJuiz.getJuiz());
        //viewDadosJuiz.preencherJuiz();
        viewDadosJuiz.setVisible(true);
        try {
            viewDadosJuiz.setSelected(true);
        } catch (PropertyVetoException ex) {
            ViewUtil.addMsgErro("Não foi possível abrir a ViewDadosJuiz");
        }
    }

    public void listar(List juizes) {
        try {
            this.juizes = juizes;
            DefaultTableModel model = (DefaultTableModel) tJuizes.getModel();
            this.removerLinhasDaTabela(model);

            Iterator resultado = juizes.iterator();

            while (resultado.hasNext()) {
                Juiz juizTemp = (Juiz) resultado.next();
                String nome = juizTemp.getNome();
                String login = juizTemp.getLogin();
                String email = juizTemp.getEmail();

                Object[] linha = {nome, login, email};
                model.addRow(linha);
            }

        } catch (RuntimeException ex) {
            ViewUtil.addMsgErro("Ocorreu um erro ao carregar os Juízes " + ex.getMessage());
        }
    }
    ...
    public Juiz getJuiz() {
        ...
        try {
            ...
            juizTemp = (Juiz) this.juizes.get(linhaSelecionada);
    ...
    private void bNovoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bNovoActionPerformed
        ...
        Juiz juiz = new Juiz();
        this.abrirViewDadosJuiz(juiz, "novo");
    ...
    private void bAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bAlterarActionPerformed
        ...
        try {
            juiz = this.getJuiz();
            this.abrirViewDadosJuiz(juiz, "editar");

        } catch (RuntimeException ex) {
            ViewUtil.addMsgErro("Não foi possível carregar os dados do Juiz " + ex.getMessage());
        }
