 ...
 * @author Gustavo
 */
public class ViewPesquisaEmpresa extends javax.swing.JInternalFrame {

    private ViewDadosEmpresa viewDadosEmpresa;
     ...
     */
    public ViewPesquisaEmpresa() {
        initComponents();
    }

    public ViewPesquisaEmpresa(Jogador jogador) {
        initComponents();
    ...
    private void abrirViewDadosEmpresa(Empresa empresa, String acao) {
        // if (viewDadosJogador == null) {
        viewDadosEmpresa = new ViewDadosEmpresa();
        OuvinteViewDadosEmpresa ouvinte = new OuvinteViewDadosEmpresa(viewDadosEmpresa, this.usuario);
        viewDadosEmpresa.setAcao(acao);
        this.getParent().add(viewDadosEmpresa);
        viewDadosEmpresa.setPosicao();
        //}
        if (acao.equals("novo")) {
            viewDadosEmpresa.bEditar.setEnabled(false);
            viewDadosEmpresa.bExcluir.setEnabled(false);
            viewDadosEmpresa.bGravar.setEnabled(true);
            viewDadosEmpresa.bNovo.setEnabled(true);
        }
        if (acao.equals("editar")) {
            viewDadosEmpresa.bGravar.setEnabled(false);
            viewDadosEmpresa.bExcluir.setEnabled(false);
            viewDadosEmpresa.bEditar.setEnabled(true);
            viewDadosEmpresa.bNovo.setEnabled(false);
        }
        if (acao.equals("excluir")) {
            viewDadosEmpresa.bExcluir.setEnabled(true);
            viewDadosEmpresa.bEditar.setEnabled(false);
            viewDadosEmpresa.bGravar.setEnabled(false);
            viewDadosEmpresa.bNovo.setEnabled(false);
        }

        viewDadosEmpresa.setEmpresa(empresa);
        viewDadosEmpresa.exibirEmpresa(viewDadosEmpresa.getEmpresa());
        //viewDadosJuiz.preencherJuiz();
        viewDadosEmpresa.setVisible(true);
        try {
            viewDadosEmpresa.setSelected(true);
        } catch (PropertyVetoException ex) {
            ViewUtil.addMsgErro("Não foi possível abrir a viewDadosEmpresa");
        }
    }

    public void listar(List empresas) {
        try {
            ...
            while (resultado.hasNext()) {
                Empresa empresaTemp = (Empresa) resultado.next();
                String nome = empresaTemp.getNomeFantasia();
                String participante = empresaTemp.getJogador().getNome();
                //String fornecedor = empresaTemp.getFornecedor().getFantasia();
                // String transportadora = empresaTemp.getTransportadora().getnomeFantasia();
                //Long rodada = empresaTemp.getRodada().getId();

                Object[] linha = {nome, participante};
                model.addRow(linha);
            }
    ...
    public Empresa getEmpresa() {
        ...
        try {
            ...
            empresaTemp = (Empresa) this.empresas.get(linhaSelecionada);
    ...
    private void bNovoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bNovoActionPerformed
        ...
        empresa.setCapitalAtual(BigDecimal.ZERO);
        empresa.setCapitalInicial(BigDecimal.ZERO);
        empresa.setMargeLucro(BigDecimal.ZERO);
        this.abrirViewDadosEmpresa(empresa, "novo");
    ...
    private void bAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bAlterarActionPerformed
        ...
        try {
            ...
            if (empresa != null) {
                this.abrirViewDadosEmpresa(empresa, "editar");
            }
        } catch (RuntimeException ex) {
